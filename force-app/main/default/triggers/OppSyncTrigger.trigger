trigger OppSyncTrigger on Opportunity (after update) {
    ...
    // bloque para filtrar opportunidades que si necesitan el trigger
    List<Opportunity> oppsToHandle = new List<Opportunity>();
    List<String> oppStagesToIgnore = new List<String>{
        'P5',
        'PU',
        'PSP',
        'PSI',
    ...
    for (Opportunity opp : trigger.new) {
          if(!oppStagesToIgnore.contains(opp.StageName)){
            oppsToHandle.add(opp);
          }
    }
    ...
    for (Opportunity opp : oppsToHandle) {
        ...
        System.debug('newOpp.q: ' + opp.SyncedQuoteId);
        System.debug('oldOpp.q: ' + oldOpp.SyncedQuoteId);
        if (opp.SyncedQuoteId != oldOpp.SyncedQuoteId) {
    ...
    if (!startSyncQuoteMap.isEmpty()) {
        ...
        for (Id quoteId : startSyncQuoteMap.keySet()) {
                    ...
                    for (OpportunityLineItem oli : opplines) {
                        if (oli.pricebookentryid == qli.pricebookentryId && oli.UnitPrice == qli.UnitPrice && oli.Quantity == qli.Quantity && oli.Discount == qli.Discount && oli.SortOrder == qli.SortOrder
                            ...
                            for (String qliField : quoteLineFields) {
                                ...
                                Object oliValue = oli.get(oliField); Object qliValue = qli.get(qliField);
                                if (oliValue != qliValue) {
                                    hasChange = true; if (qliValue == null){ oli.put(oliField, null); }  else { oli.put(oliField, qliValue);}
                                }    
                            }
         ...
         if (!updateOliSet.isEmpty()) {
             updateOliList.addAll(updateOliSet);TriggerStopper.stopQuote = true;             TriggerStopper.stopOppLine = true;TriggerStopper.stopQuoteLine = true;             Database.update(updateOliList);updateOliSet.clear();updateOliList.clear();TriggerStopper.stopQuote = false;             TriggerStopper.stopOppLine = false;TriggerStopper.stopQuoteLine = false;                               
         }                
    }
